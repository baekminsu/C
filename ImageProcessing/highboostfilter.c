...
main() {
	...
	mask[1][0] = -1.0 / 9.0; mask[1][1] = (1.0 + (1.0 - 1.0 / 9.0)); mask[1][2] = -1.0 / 9.0;
	mask[2][0] = -1.0 / 9.0; mask[2][1] = -1.0 / 9.0; mask[2][2] = -1.0 / 9.0;
	double ff;
		for(ii1=0;ii1<fvd-1;ii1++){
			for(ii2=0;ii2<fhd-1;ii2++){
				ff = 0.0;
				for (ii3 = -1; ii3 <= 1; ii3++) {
					for (ii4 = -1; ii4 <= 1; ii4++) {
						ff = ff + mask[ii3 + 1][ii4 + 1] = (float)cc[ii1 + ii3][ii2 + ii4];
					}
				}
				if (ff > 255.0) {
					ff = 255.0;
